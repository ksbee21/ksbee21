...
# WebGL 에서 Camera 란 ?
   ...
   그래서 우리가 그릴 공간에 표현하고자 하는 물체를 아 겹치게 위치를 조정해 놓는 작업을 하게 됩니다.    
   그 방법이 물체마다, 왼쪽 오른쪽으로 이동( x position) ,  위로 아래로 이동 ( y position ) , 뒤로 앞으로 이동 ( z position ) 하면
   물체끼리 겹쳐서 안나타 나는 일이 없도록 구성해 볼 수 있습니다.   
   어떤 공간에 물체 하나 하나의 위치를 배치해 주는 역확을 담당하는것이 worldMatrix 의 역활입니다.     
   당연히 물체 하나 하나 달리 적용이 되어야 합니다.    
